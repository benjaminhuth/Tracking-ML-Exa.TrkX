...
def local_to_global(measurements, detector):
    global_pos = np.zeros((len(measurements.index), 3))
    geoid_to_transform = get_transforms(detector)
...
def get_layerwise_edges(hits):
    ...
    hit_list = (
        hits.groupby(["particle_id", "volume_id", "layer_id"], sort=False)["index"]
        .agg(lambda x: list(x))
        .groupby(level=0)
        .agg(lambda x: list(x))
    )
...
def build_event(
    ...
    # Take information from detector and particles
    hits = hits.merge(
